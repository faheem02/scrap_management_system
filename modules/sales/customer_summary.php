...
$sql = "SELECT s.invoice_no, s.sale_date, s.total_amount, s.paid_amount, s.due_amount,
               c.id AS customer_id, c.full_name, c.plant_area, c.phone
        FROM sales s
...
foreach ($sales as $s) {
    ...
    if (!isset($groups[$key])) {
        $groups[$key] = ['name' => $s['full_name'] ?: 'Walk-in Customer', 'area' => $s['plant_area'], 'phone' => $s['phone'], 'rows' => []];
    }
    $groups[$key]['rows'][] = $s;
...
<div class="card shadow">
  ...
  <div class="card-body">
    ...
    <form method="get" class="row g-2 mb-3 d-print-none">
      ...
      <div class="col-md-2">
        <?php if ($from || $to): ?>
        <a href="customer_summary.php" class="btn btn-outline-secondary btn-block"><i class="fas fa-times"></i> Reset</a>
        <?php endif; ?>
      </div>
    </form>
